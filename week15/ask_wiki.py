...
SYSTEM_V1 = """너는 적설계 시스템(SnowViewer3D) 기술 안내 봇이다.
반드시 한국어로만 답한다.
...
# v2: 환각 방지 장치가 너무 세서 정답까지 막던 문제 대응.
# 59문항 진단에서 3건 중 2건이 "정답 문서를 근거 1로 받고도 찾을 수 없다고 답함"이었다.
# 원인은 답이 표 안에 있어 문장으로 쓰여 있지 않았던 것.
SYSTEM_V2 = """너는 적설계 시스템(SnowViewer3D) 기술 안내 봇이다.
반드시 한국어로만 답한다.

답하는 규칙:
- 제공된 근거만 사용한다. 근거 밖의 지식으로 지어내지 않는다.
- 근거의 표, 목록, 코드에서 읽어낼 수 있는 내용도 답한다. 문장으로 풀어 쓰여 있지
  않더라도 표의 행과 열을 짚어 읽으면 그것도 근거다.
- 답이 근거에 일부만 있으면, 있는 만큼 답하고 없는 부분만 따로 밝힌다.
- 근거를 끝까지 읽어도 답이 없을 때만 "자료에서 찾을 수 없습니다"라고 답한다.
  근거에 있는데 포기하는 것도 틀린 답이다.
- 어느 근거를 참고했는지 (근거 1) 형식으로 표시한다.
- 숫자, 파일명, 코드 위치는 근거에 적힌 그대로 옮긴다."""

# $env:ASK_PROMPT = "v1" 로 되돌릴 수 있다
SYSTEM = SYSTEM_V1 if os.environ.get("ASK_PROMPT") == "v1" else SYSTEM_V2
...
# 16주차: LLM 리랭커를 붙여 재봤으나 기각했다. 다시 시도하기 전에 아래를 볼 것.
#   week16/retrieval/hybrid.json  vs  hybrid-rerank.json
#
#                   하이브리드   +리랭커(gemma4:e4b)
#     Recall@1        87.5%        78.6%   <- 떨어짐
#     MRR             0.9222       0.8761  <- 떨어짐
#     문항당          0.19초       19.6초  <- 100배 느림
#
#   겨냥했던 어휘 충돌 2건(6위->1위, 7위->2위)은 실제로 고쳐졌다. 그런데 원래 1위였던
#   8건을 뒤로 밀어내 총점이 손해였다. 리랭커가 검색기보다 관련도 판단을 잘해야 의미가
#   있는데, 유효 4.5B 모델은 bge-m3 + BM25 조합보다 못했다.
#   다시 한다면 범용 LLM이 아니라 전용 cross-encoder(bge-reranker 계열)로 할 것.
